# Wheat Yield Forecasting System

A comprehensive wheat yield forecasting system for Argentine agricultural fields using Sentinel-2 satellite imagery, meteorological data, and phenological modeling.

## 🌾 Overview

This system provides accurate wheat yield predictions by:
...
cd Las-Petacas-yield-forecasting
```
...
The `sowing_date` and `wheat_variety` in properties are optional - the CLI will prompt for them if missing.
...
- `test_sentinel_connection.py` - Test Sentinel Hub connection
- `test_sentinel_per_date.py` - Test per-date NDVI fetching
